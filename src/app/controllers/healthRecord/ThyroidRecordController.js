...
class ThyroidRecordController {
    save = async (req, res) => {
        try {
            const authHeader = req.headers.authorization;
            const canAccess = await authenticateToken(authHeader);
            if (canAccess == 1) {
                let newHealthRecord = await ThyroidRecordModel.findById(
                    req.body?._id,
                );
                // console.log(newHealthRecord)
                if (!newHealthRecord) {
                    console.log('no record');
                    newHealthRecord = new ThyroidRecordModel();
                }
    ...
    getById = async (req, res) => {
        try {
            ...
            const canAccess = await authenticateToken(authHeader);
            if (canAccess == 1) {
                const record = await ThyroidRecordModel.findById(req.body.id);
    ...
    deleteById = async (req, res) => {
        try {
            ...
            if (canAccess == 1) {
                ...
                await ThyroidRecordModel.deleteOne({ _id: req.body.id });
                return res
                    .status(200)
    ...
    getAll = async (req, res) => {
        try {
            ...
            if (canAccess == 1) {
                const records = (
                    await ThyroidRecordModel.find({}).select(
...
module.exports = new ThyroidRecordController();
